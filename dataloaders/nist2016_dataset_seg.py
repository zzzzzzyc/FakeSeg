...
class NISTNC2016ValDataset(Dataset):
    ...
    def __init__(
        ...
        self.osn_dir = (
            os.path.join(base_dir, osn_root, f"NIST16_{osn_platform}")
            if osn_platform
            else None
        )
        ...
        if os.path.isdir(self.osn_dir):
            ...
            for root, _, files in os.walk(self.osn_dir):
                for file_name in files:
                    ...
                    if file_name.lower() == name_lower or (
                        file_stem.lower() == stem_lower and file_ext.lower() in image_exts
                    ):
    ...
    def _collect_samples(self):
        ...
        for task in self.tasks:
            ...
            for row in rows:
                ...
                eval_image_path = self._resolve_osn_image(image_path)
    ...
    def __getitem__(self, idx: int):
        ...
        original_image_path = sample.get("original_image_path", image_path)
        mask_path = sample["mask_path"]
        use_prior = sample.get("use_prior", True)
