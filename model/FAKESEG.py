...
class SesameMetaModel(nn.Module):
    ...
    def initialize_sesame_modules(self, config):
        ...
        self.sim_modulation_a = nn.Parameter(
            torch.tensor([1.0], dtype=torch.float32)
        )
        self.sim_modulation_b = nn.Parameter(
...
class SESAMEForCausalLM(LlavaLlamaForCausalLM):
    ...
    def _extract_last_layer_seg_features(
        self,
        all_hidden_states,
        ...
        last_hidden_state = (
            all_hidden_states[-1]
            if isinstance(all_hidden_states, (list, tuple))
            else all_hidden_states
        )
        device = last_hidden_state.device
        batch_size, _, hidden_dim = last_hidden_state.shape

        seg_token_mask_bool = seg_token_mask.bool()
        seg_token_embeds = last_hidden_state[seg_token_mask_bool]

        image_token_mask = input_ids == IMAGE_TOKEN_INDEX
        image_token_idx = image_token_mask.float().masked_fill(
            ~image_token_mask, float("inf")
        ).argmin(dim=1)

        idx_offset = torch.arange(num_tokens_per_image, device=device).unsqueeze(0).expand(batch_size, -1)
        gather_idx = image_token_idx.unsqueeze(1) + idx_offset
        gather_idx = gather_idx.unsqueeze(-1).expand(-1, -1, hidden_dim)
        image_token_embeds = torch.gather(last_hidden_state, dim=1, index=gather_idx)

        all_batch_indices = torch.arange(batch_size, device=device).unsqueeze(1).expand_as(seg_token_mask)
        batch_idx = all_batch_indices[seg_token_mask_bool]
        seg_image_token_embeds = image_token_embeds[batch_idx]

        return seg_token_embeds, seg_image_token_embeds, seg_token_embeds

    def _modulate_similarity_with_local_prior(
        self,
        similarity: torch.Tensor,
        seg_token_mask: torch.Tensor,
        seg_token_embeds: torch.Tensor,
        prior_fg: Optional[torch.Tensor],
        prior_bg: Optional[torch.Tensor],
    ):
        if similarity.numel() == 0 or prior_fg is None or prior_bg is None:
            return similarity

        similarity = similarity.float()
        seg_token_mask_bool = seg_token_mask.bool()
        all_batch_indices = torch.arange(seg_token_mask.shape[0], device=seg_token_mask.device).unsqueeze(1).expand_as(seg_token_mask)
        batch_idx = all_batch_indices[seg_token_mask_bool]

        side = int(similarity.shape[1] ** 0.5)
        if side * side != similarity.shape[1]:
            return similarity.to(dtype=seg_token_embeds.dtype)

        prior_fg_patch = self._pool_prior_to_patch_grid(prior_fg, side)
        prior_bg_patch = self._pool_prior_to_patch_grid(prior_bg, side)
        seg_prior_fg = prior_fg_patch[batch_idx]
        seg_prior_bg = prior_bg_patch[batch_idx]

        alpha = self.model.sim_modulation_a.float().view(1, 1)
        beta = self.model.sim_modulation_b.float().view(1, 1)
        gate = (alpha * seg_prior_fg) / (alpha * seg_prior_fg + beta * seg_prior_bg + 1e-6)
        gate = gate.clamp(min=0.0, max=1.0)
        modulated_similarity = similarity * (1.0 + 0.5 * (2.0 * gate - 1.0))
        return modulated_similarity.to(dtype=seg_token_embeds.dtype)

    def _pool_prior_to_patch_grid(self, prior, side: int) -> torch.Tensor:
        if isinstance(prior, list):
            ...
            for item in prior:
                ...
                if item.dim() == 3:
                    item = item.unsqueeze(0)
                pooled = F.adaptive_avg_pool2d(item.float(), (side, side)).flatten(1)
                pooled_list.append(pooled.squeeze(0))
            return torch.stack(pooled_list, dim=0)

        if prior.dim() == 3:
            prior = prior.unsqueeze(1)
        return F.adaptive_avg_pool2d(prior.float(), (side, side)).flatten(1)
    ...
    def generate_pred_masks(self, pred_embeddings, image_embeddings, sam_mask_shape_list, similarity):
        ...
        for i in range(len(pred_embeddings)):
            ...
            #     similarity[i],
            #     sam_mask_shape_list[i][1],
            #     target_length=336,
            # )
            # prompt_mask = self.get_region_prompt_map(
            #     similarity[i],
            #     sam_mask_shape_list[i][1],
            #     target_length=336,
            #     sam_mask_size=256,
    ...
    def model_forward(
        ...
        **kwargs,
        ...
        prior_fg = kwargs.get("prior_fg")
        prior_bg = kwargs.get("prior_bg")
        ...
        seg_token_embeds_for_similarity, seg_image_token_embeds_for_similarity, \
            seg_token_embeds_for_sam = self._extract_last_layer_seg_features(
        ...
        for batch_idx in range(len(pred_masks)):
            ...
            sm_for_supervision = self.similarity_for_supervision[batch_idx]
            non_empty_mask = gt_mask.sum(dim=(1, 2)) >= 1.0
            non_empty_indices = torch.where(non_empty_mask)[0]
            gt_mask = gt_mask[non_empty_indices]
            pred_mask = pred_mask[non_empty_indices]
            sm_for_supervision = sm_for_supervision[non_empty_indices]
    ...
    def compute_similarity(
        ...
        similarity = self._modulate_similarity_with_local_prior(
            similarity,
            seg_token_mask,
            seg_token_embeds,
            prior_fg,
    ...
    def get_similarity_map(self, sm, shape, target_length=336):
        ...
        sm = F.interpolate(sm, (target_length, target_length), mode="bilinear", align_corners=False)
        ...
        sm = sm[:, :, 0:newh, 0:neww]
        sm = F.interpolate(sm, shape, mode="bilinear", align_corners=False)
    ...
    def similarity_to_spatial_map(self, sm, shape, target_length=336):
        # sm: [num_obj, N_patch]
        sm = sm.float()
        sm = (sm - sm.min(1, keepdim=True)[0]) / (
            sm.max(1, keepdim=True)[0] - sm.min(1, keepdim=True)[0] + 1e-6
        ...
        side = int(sm.shape[1] ** 0.5)
        sm = sm.reshape(sm.shape[0], side, side).unsqueeze(1)  # [num_obj,1,side,side]
        sm = F.interpolate(sm, (target_length, target_length), mode="bilinear", align_corners=False)
        ...
        sm = F.interpolate(sm, shape, mode="bilinear", align_corners=False)
        self.similarity_for_supervision.append(sm.squeeze(1))
        return sm  # [num_obj,1,H,W]

    def spatial_map_to_region_prompt(
        self,
        spatial_map: torch.Tensor,
        sam_mask_size: int = 256,
        grid_size: int = 16,
        top_n_cell: int = 36,
    ):
        # spatial_map: [num_obj,1,H,W]
        num_obj, _, H, W = spatial_map.shape
        device = spatial_map.device
        dtype = spatial_map.dtype

        grid_map = F.adaptive_avg_pool2d(spatial_map.float(), (grid_size, grid_size))
        flat_grid = grid_map.flatten(1)  # [num_obj, grid_size*grid_size]
        total_cells = flat_grid.shape[1]
        topk = min(max(1, top_n_cell), total_cells)

        topk_idx = torch.topk(flat_grid, k=topk, dim=1, largest=True).indices
        binary_grid = torch.zeros_like(flat_grid)
        binary_grid.scatter_(1, topk_idx, 1.0)
        binary_grid = binary_grid.view(num_obj, 1, grid_size, grid_size)

        prompt_mask = F.interpolate(
            binary_grid,
            size=(sam_mask_size, sam_mask_size),
            mode="nearest",
        ).to(device=device, dtype=dtype)
        return prompt_mask  # [num_obj,1,256,256]

    def get_region_prompt_map(
        self,
        sm,
        shape,
        target_length=336,
        ...
        spatial_map = self.similarity_to_spatial_map(sm, shape, target_length=target_length)
        prompt_mask = self.spatial_map_to_region_prompt(
            spatial_map,
            sam_mask_size=sam_mask_size,
            grid_size=grid_size,
            top_n_cell=top_n_cell,
        )
        return prompt_mask
    ...
    def _extract_seg_image_token(
        ...
        seg_token_embeds = hidden_states[seg_token_mask.bool()]  # [N_seg, D]
        image_token_mask = (input_ids == IMAGE_TOKEN_INDEX)  # [B, T]
        image_token_idx = (input_ids == IMAGE_TOKEN_INDEX).float().masked_fill(~image_token_mask, float('inf')).argmin(
            dim=1)  # [B]
        ...
        all_batch_indices = torch.arange(B, device=seg_token_mask.device).unsqueeze(1).expand_as(
            seg_token_mask)  # [B, T]
        batch_idx = all_batch_indices[seg_token_mask.bool()]  # [N_seg]
        seg_image_token_embeds = image_token_embeds[batch_idx]  # [N_seg, 256, D]
    ...
    def evaluate(
        ...
        **kwargs
        ...
        with torch.inference_mode():
            ...
            assert len(self.model.text_hidden_fcs) == 1
            seg_token_embeds_for_similarity, seg_image_token_embeds_for_similarity, seg_token_embeds_for_sam = self._extract_last_layer_seg_features(
                output_hidden_states,
                input_ids,
                seg_token_mask,
                num_tokens_per_image,
