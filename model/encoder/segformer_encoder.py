...
class XTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dims=[64, 128, 256, 512],
                 ...
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm, depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], stride0=4):
        super().__init__()

        # ---- Patch Embedding ----
        self.patch_embed1 = OverlapPatchEmbed(
            img_size=img_size, patch_size=7, stride=stride0,
    ...
    def init_weights_onepath(self, pretrained=None):
        if isinstance(pretrained, str):
            load_singlepath_model(self, pretrained)
        else:
...
def load_singlepath_model(model, model_file):
